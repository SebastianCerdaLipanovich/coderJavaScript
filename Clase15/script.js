...
$(() => {
        ...
        document.getElementById("btnCrearAlumno").onclick = () => {
            ...
            if (nombre && apellido && carrera) {
                if (localStorage.getItem('alumno')) {
                    $("body").fadeOut(1000, function () {
                        location.href = "/Clase15/views/examenes.html";
                    });
    ...
    $("#mensajeBorrarNotas").html(`Haz click <a id="borraNotas" href="#">aqui</a> para borrar todas las notas`);
    $("#borraNotas").on("click", () => {
        let alumnoSinNotas = JSON.parse(localStorage.getItem("alumno"));
        alumnoSinNotas.notas = []
        localStorage.setItem("alumno", JSON.stringify(alumnoSinNotas));
        $("body").fadeOut(1000, function () {
            location.reload();;
        });
    })
    ...
    if ($("#home")) {
        $("#home").on("click", () => {
            $("body").fadeOut(1000, function () {
                location.href ="/Clase15/index.html"
            });
        })
    }
